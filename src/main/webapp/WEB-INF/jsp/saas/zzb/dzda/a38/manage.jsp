...
<div class="portlet-title">
    ...
    <div class="clearfix fr">
        <c:if test="${sjztManage eq '0'}">
            <button type="button" class="btn green" onclick="formSave('0')"><i class="icon-question-sign"></i> 保存
            </button>
...
<div class="tabbable tabbable-custom" style="margin-bottom: 0px">
    ...
    <div class="tab-content" style="border:none; border-top:solid 1px #e4e4e4; padding:5px 0;">
        <input type="hidden" id="isDacx" name="isDacx" value="${isDacx}">
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $('a[data-toggle="tab"]').on('show.bs.tab', function (e) {
            if ($(e.target).attr('id') != '#tab_1_1' && tabIndex == "#tab_1_1" && ${isDacx!='1'}) {
    ...
    function fileDownManage(type) {
        if (type == "danganxiazai") {
            window.open("${path}/zzb/dzda/a38/download/${id}?a0101=${a0101}&OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}");
        }
        if (type == "dangantupianxiazai") {
            window.open("${path}/zzb/dzda/mlcl/tpcl/download/${id}?a0101=${a0101}&OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}");
        }
        if (type == "qianquecail") {
            window.open("${path}/zzb/dzda/e01z4/download/${id}?a0101=${a0101}&OWASP_CSRFTOKEN=${sessionScope.OWASP_CSRFTOKEN}");
        }
